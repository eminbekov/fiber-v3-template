...
- [x] `Cache` interface + Redis implementation (guide §14, §25).
- [x] Type-safe cache keys (guide §14.2).
- [x] Cache-aside in services; invalidation strategy (guide §14.4–§14.5).
- [x] If using admin HTML sessions: Redis session store (guide §15.3).
...
- [ ] NATS connection and JetStream patterns (guide §16, §25).
- [ ] Background consumers under `internal/nats/consumers` (guide §2).
- [ ] `proto/` definitions and generated `gen/` code; gRPC server alongside Fiber (guide §17, §25).
- [ ] gRPC interceptors (recovery, OTEL) (guide §17.4–§17.5).
